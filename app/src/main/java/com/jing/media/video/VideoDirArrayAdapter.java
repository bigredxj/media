...
public class VideoDirArrayAdapter extends ArrayAdapter<VideoDir> {
    ...
    public VideoDirArrayAdapter(Context context, int resource, List<VideoDir> objects) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = LayoutInflater.from(getContext()).inflate(R.layout.video_dir_item, parent, false);
        }

        // Set up the TextView and Button
        TextView name = (TextView) convertView.findViewById(R.id.video_dir_name_tv);
        TextView size = (TextView) convertView.findViewById(R.id.video_dir_size_tv);

        // Get the item
        final VideoDir item = getItem(position);

        // Set the text for the TextView
        name.setText(item.getName());
        size.setText("(共"+item.getSize()+"集)");
